...
pub struct TranslationPolicy {
    ...
    /// Fixed OpenRouter `provider` preferences object, injected into every
    /// outgoing request regardless of what Claude Code sent. See
    /// OPENROUTER_PROVIDER_PREFERENCES in config.rs.
    pub provider_preferences: Option<Value>,
}

/// Claude Code always sends `metadata.user_id` as a stable-per-session
/// string (in practice a JSON blob of its own: device_id/account_uuid/
/// session_id — Anthropic's spec requires `user_id` to be a plain string,
/// so that's where Claude Code packs its session tracking). Whatever the
/// exact contents, it stays byte-identical across every request in one
/// Claude Code session, which is all OpenRouter's sticky routing needs.
pub fn extract_session_id(req: &anthropic::AnthropicRequest) -> Option<String> {
    req.metadata
        .as_ref()
...
pub fn translate_request(
    ...
    Ok(openai::OpenAIRequest {
        ...
        tools,
        tool_choice: None,
        provider: policy.provider_preferences.clone(),
        session_id,
    })
}
...
mod tests {
    ...
    #[test]
    fn injects_fixed_provider_preferences_regardless_of_request_content() {
        ...
        let policy = TranslationPolicy {
            provider_preferences: Some(json!({"only": ["deepseek/fp8", "baseten/fp8"]})),
            ..default_policy()
        };
        ...
        assert_eq!(
            openai.provider,
            Some(json!({"only": ["deepseek/fp8", "baseten/fp8"]}))
        );
    }

    #[test]
    fn omits_provider_field_when_no_preferences_configured() {
        let req = anthropic::AnthropicRequest {
            model: "claude-opus-4-6".to_string(),
            messages: vec![anthropic::Message {
                role: "user".to_string(),
        ...
        assert_eq!(openai.provider, None);

        // Confirm it's actually skipped from serialization, not just null.
        let serialized = serde_json::to_value(&openai).unwrap();
        assert!(!serialized.as_object().unwrap().contains_key("provider"));
    }
