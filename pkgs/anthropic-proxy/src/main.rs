...
async fn async_main(cli: Cli) -> anyhow::Result<()> {
    ...
    let routing_state = Arc::new(routing::RoutingState::new(
        config
    ...
    if let Some(ref model) = config.provider_tracking_model {
        tracing::info!(
            "Dynamic provider routing enabled for {} (min_throughput={} tok/s, max_latency={}ms)",
            model,
            config.provider_min_throughput,
            config.provider_max_latency_ms
        );
