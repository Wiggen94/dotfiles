...
pub async fn proxy_handler(
    ...
    Extension(routing_state): Extension<Arc<RoutingState>>,
    headers: HeaderMap,
    ...
    let session_id = pipeline::extract_session_id(&req);
    let mut policy = translation_policy(&config);
    if let Some(dynamic_tags) = routing_state.allowed_tags_for(session_id.as_deref()) {
        // Dynamic session-frozen allowlist wins over any static `only` from
        // OPENROUTER_PROVIDER_PREFERENCES; other static fields (e.g. sort)
        ...
        let mut merged = policy
            ...
            .unwrap_or_default();
        merged.insert("only".to_string(), json!(dynamic_tags));
        policy.provider_preferences = Some(Value::Object(merged));
    }
    ...
    let result = if is_streaming {
        handle_streaming(config, client, routing_state, openai_req, api_key).await
    } else {
        handle_non_streaming(config, client, routing_state, openai_req, api_key).await
    };
...
fn translation_policy(config: &Config) -> pipeline::TranslationPolicy {
    pipeline::TranslationPolicy {
        ...
        provider_preferences: config.provider_preferences.clone(),
    }
}
...
/// Fire-and-forget: don't hold up the response to Claude Code waiting on
/// OpenRouter's generation-stats endpoint (it's best-effort, and has a
/// natural delay before results are available — see routing.rs).
fn spawn_observation(
    ...
    tokio::spawn(async move {
        routing::record_observation_from_generation(routing_state, client, api_key, generation_id)
            .await;
    });
}
...
async fn handle_non_streaming(
    ...
    for url in &urls {
        ...
        if let Some(ref generation_id) = openai_resp.id {
            spawn_observation(&routing_state, &client, &api_key, generation_id.clone());
        }
...
async fn handle_streaming(
    ...
    for url in &urls {
        ...
        let routing_state_for_hook = routing_state.clone();
        let client_for_hook = client.clone();
        let api_key_for_hook = api_key.clone();
        let sse_stream = create_sse_stream(upstream, openai_req.model.clone(), move |generation_id| {
            spawn_observation(
                &routing_state_for_hook,
                &client_for_hook,
                &api_key_for_hook,
                generation_id,
            );
...
fn create_sse_stream(
    ...
    on_first_id: impl Fn(String) + Send + 'static,
...
mod tests {
    ...
    async fn collect_events(chunks: Vec<String>, model: &str) -> Vec<Value> {
        ...
        let sse = create_sse_stream(s, model.to_string(), |_| {});
        tokio::pin!(sse);
    ...
    async fn stream_error_produces_error_event_and_stops() {
        ...
        let sse = create_sse_stream(s, "fallback".to_string(), |_| {});
        tokio::pin!(sse);
